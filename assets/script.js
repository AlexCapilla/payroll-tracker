...
const collectEmployees = function () {
  ...
  while (letsKeepGoing) {
    ...
    letsKeepGoing = confirm("Do you want to add another employee?");
  }
...
const displayAverageSalary = function (employeesArray) {
  ...
  console.log(
    `Average employee salary between our ${numEmployees} employee(s) is$${averageSalary.toFixed(
      2
    )}`
  );
};
...
const getRandomEmployee = function (employeesArray) {
  const randomEmployee =
    employeesArray[Math.floor(Math.random() * numEmployees)];
  console.log(
    `Random Employee: ${randomEmployee.firstName} ${randomEmployee.lastName} is our daily winner!`
  );
};
